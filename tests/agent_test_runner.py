from agent_protocol import IGuesser, IAsyncGuesser
...
async def test_agent_async(agent: IAsyncGuesser, secret_code: str | None = None,
    ...
    Args:
        agent: The async agent to test (must implement IAsyncGuesser protocol)
        secret_code: The secret code to guess. If None, a random code is generated.
        max_attempts: Maximum number of attempts the agent can make.
    ...
    for attempt in range(max_attempts):
        ...
        feedback = evaluate_guess(guess.guess, secret_code)
        await agent.provide_feedback(feedback)
        logger.info(f"Feedback: {feedback}")
...
async def test_agent_simplified_async(agent: IAsyncGuesser, secret_code: str | None = None,
    ...
    for attempt in range(max_attempts):
        ...
        # modified feedback for compatibility with the agent protocol
        mod_feedback = (feedback, 0)
        await agent.provide_feedback(mod_feedback)
        is_won = feedback == 4
        logger.info(f"Feedback: {mod_feedback}")
        logger.info(f"Is won: {is_won}")
        if is_won:
